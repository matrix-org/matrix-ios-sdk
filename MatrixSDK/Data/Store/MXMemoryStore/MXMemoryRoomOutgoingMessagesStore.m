...
@interface MXMemoryRoomOutgoingMessagesStore ()
{
}

@end

@implementation MXMemoryRoomOutgoingMessagesStore
...
- (void)removeOutgoingMessage:(NSString*)eventId
{
    for (NSUInteger i = 0; i < outgoingMessages.count; i++)
    {
        ...
        if ([outgoingMessage.eventId isEqualToString:eventId])
        {
...
{
    return [NSString stringWithFormat:@"<%@: %tu messages>", NSStringFromClass(self.class), outgoingMessages.count];
}
